...
def weight_territory(territories: gpd.GeoDataFrame, 
                     railway_stops: gpd.GeoDataFrame, 
                     bus_stops: gpd.GeoDataFrame, 
                     ferry_stops: gpd.GeoDataFrame, 
                     airports: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    ...
    territories_with_r_stops = territories.sjoin_nearest(railway_stops, distance_col='distance_to_railway_stops')
    territories_with_b_stops = territories.sjoin_nearest(bus_stops, distance_col='distance_to_bus_stops')
    territories_with_ferry = territories.sjoin_nearest(ferry_stops, distance_col='distance_to_ferry_stops')
    territories_with_airports = territories.sjoin_nearest(airports, distance_col='distance_to_airports')

    # Initialize weights
    territories['weight'] = 0.0

    # Calculate weights based on distances
    territories['weight'] += territories_with_r_stops['distance_to_railway_stops'].apply(lambda x: 0.35 if x <= MAX_DISTANCE else 0.0)
    territories['weight'] += territories_with_b_stops['distance_to_bus_stops'].apply(lambda x: 0.35 if x <= MAX_DISTANCE else 0.0)
    territories['weight'] += territories_with_ferry['distance_to_ferry_stops'].apply(lambda x: 0.20 if x <= MAX_DISTANCE else 0.0)
    territories['weight'] += territories_with_airports['distance_to_airports'].apply(lambda x: 0.10 if x <= MAX_DISTANCE else 0.0)
...
def assign_grades(graded_territories: gpd.GeoDataFrame, accessibility_data: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    ...
    joined = gpd.sjoin(graded_territories, accessibility_data, how="left", predicate='intersects')

    # Calculate the area of intersection
    joined['intersection_area'] = joined.apply(
        lambda row: row['geometry'].intersection(accessibility_data.loc[row['index_right'], 'geometry']).area, axis=1
    )
    ...
    for idx, row in joined.iterrows():
        ...
        public_transport_grade = public_transport_grade_table.get(grade, {}).get(f'Q{public_transport_quartile}', 0) * row['weight']
    ...
    return joined[['geometry', 'grade', 'weight', 'car_access_quartile', 'public_access_quartile', 'car_grade', 'public_transport_grade', 'overall_assessment']]